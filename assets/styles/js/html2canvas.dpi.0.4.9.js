...
(function (window, document, undefined) {
    ...
    _html2canvas.Util.Bounds = function (element) {
        ...
        if (!element) {
            bounds.top = bounds.bottom = bounds.left = bounds.width = bounds.height = 0;
            return bounds;
        }
   ...
   */
    ...
    window.html2canvas = function (elements, opts) {
        ...
        if (options["width"]) {
            options["width"]  = (options["width"].indexOf("%") !== -1) ? container.width() * parseFloat(options["width"]) / 100 : options["width"];
        } else {
            options["width"] = $(options.elements[0]).width();
        }
        if (options["height"]) {
            options["height"] = (options["height"].indexOf("%") !== -1) ? container.height() * parseFloat(options["height"]) / 100 : options["height"];
        } else {
            options["height"] = $(options.elements[0]).height();
        }

        if (options["left"]) {
            options["left"] = (options["left"].indexOf("%") !== -1) ? $(elements[0]).width() * parseFloat(options["left"]) / 100 : options["left"];
        } else {
            options["left"] = 0;
        }

        if (options["top"]) {
            options["top"] = (options["top"].indexOf("%") !== -1) ? $(elements[0]).height() * parseFloat(options["top"]) / 100 : options["top"];
        } else {
            options["top"] = 0;
        }
        ...
        return function (parsedData, options, document, queue, _html2canvas) {
            ...
            if (options.elements.length === 1) {
                if (typeof options.elements[0] === "object" && options.elements[0].nodeName !== "BODY") {
                    ...
                    // Correct for scrolling when position is not fixed (iOS bouncing scroll fix)
                    var dx = 0, dy = 0;
                    if ($(container).css("position") != "fixed") {
                          dx = window.scrollX * options.scale;
                          dy = window.scrollY * options.scale;
                    }

                    ctx = newCanvas.getContext("2d");
                    if (bounds.left != bounds.right && bounds.top != bounds.bottom) {
                        ctx.drawImage(canvas, bounds.left, bounds.top, bounds.width, bounds.height, dx, dy, bounds.width, bounds.height);
                    }
